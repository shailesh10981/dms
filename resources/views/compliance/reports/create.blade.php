...
@section('title', 'Create Compliance Report')
...
<div class="container-fluid">
  <div class="row mb-4">
    <div class="col-md-6">
      ...
      <h5 class="text-muted">Template: {{ $template->name }}</h5>
    </div>
  ...
  <div class="card">
    <div class="card-body">
      <form method="POST" action="{{ route('compliance.reports.store', $template) }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group mb-4">
          ...
          <select class="form-control @error('fields.' . $field->label) is-invalid @enderror"
            ...
            <option value="{{ trim($option) }}"
              {{ old('fields.' . $field->label) == trim($option) ? 'selected' : '' }}>
              {{ trim($option) }}
            </option>
            @endforeach
          </select>
          ...
          <div class="custom-control custom-checkbox">
            <input type="checkbox"
              ...
              name="fields[{{ $field->label }}]"
              value="1"
              {{ old('fields.' . $field->label) ? 'checked' : '' }}>
          ...
          <textarea class="form-control @error('fields.' . $field->label) is-invalid @enderror"
            ...
            name="fields[{{ $field->label }}]"
            rows="3"
            @if($field->is_required) required @endif>{{ old('fields.' . $field->label) }}</textarea>
          @endif
